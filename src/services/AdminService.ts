...
export class AdminService {
    ...
    async createAdmin(data: CreateAdminData, creatorRole?: string): Promise<Admin> {
        try {
            console.log('createAdmin çağrıldı:', {
            ...
            // Sadece superadmin yeni admin ekleyebilir
            if (creatorRole !== 'superadmin') {
                console.log('Yetkisiz erişim: creatorRole =', creatorRole);
                throw new Error('Sadece superadmin rolündeki kullanıcılar yeni admin oluşturabilir');
            }

            // createInitialSuperAdmin metodu hariç, sadece admin rolü oluşturulabilir
            // İlk süper admin oluşturma dışında kullanım için kontrol
            if (data.role !== 'admin' && data.role !== 'superadmin') {
                console.log('Geçersiz rol: istenilen rol =', data.role);
                throw new Error('Sadece "admin" rolü oluşturulabilir');
            }

            // Süper admin oluşturma kontrolü - süper admin rolü sadece createInitialSuperAdmin tarafından oluşturulabilir
            if (data.role === 'superadmin') {
                console.log('Superadmin oluşturma kontrolü yapılıyor');
                // superAdmin varlığını kontrol et
                const hasSuperAdmin = await this.hasSuperAdmin();
                if (hasSuperAdmin) {
                    console.log('Sistemde zaten bir superadmin var');
                    throw new Error('Sistemde zaten bir superadmin bulunmaktadır');
                }
            }
            ...
            try {
                ...
                const admin = await prisma.$transaction(async (tx) => {
                    // Create admin
                    console.log('Admin oluşturuluyor (Prisma)');
                    ...
                    console.log('Admin başarıyla oluşturuldu (Prisma):', newAdmin.id);
                    return newAdmin;
                });

                console.log('Admin oluşturma başarılı, doğrulama yapılıyor ve dönüş hazırlanıyor');
                return this.mapAdminFromDb(admin);
            } catch (prismaError: any) {
                console.error('Prisma ile admin oluştururken hata:', prismaError);

                // Eğer unique constraint hatası varsa, özel bir hata fırlat
                if (prismaError.code === 'P2002') {
                    const target = prismaError.meta?.target?.[0];
                    console.log('Unique constraint hatası, etkilenen alan:', target);
                    ...
                    if (target === 'username') {
                        ...
                        throw new Error(`Benzersizlik hatası: ${target}`);
                    }
                }

                // Fallback to Supabase
                try {
                    console.log('Supabase fallback kullanılıyor');
                    ...
                    if (error) {
                        console.error('Supabase admin oluşturma hatası:', error);
                        // Unique constraint hatası kontrolü (Supabase)
                        if (error.code === '23505') { // PostgreSQL unique constraint violation
                            if (error.message.includes('username')) {
                                ...
                                throw new Error(`Benzersizlik hatası: ${error.message}`);
                            }
                        }
                        throw error;
                    }
                    ...
                    if (data.profile) {
                        console.log('Admin profili oluşturuluyor (Supabase)');
                        ...
                        if (profileError) {
                            console.error('Admin profili oluşturma hatası (Supabase):', profileError);
                            throw profileError;
                        }
                    }

                    // Get complete admin with profile
                    console.log('Tam admin bilgisi alınıyor (Supabase)');
                    ...
                    if (fullAdminError) {
                        console.error('Tam admin bilgisi alınırken hata (Supabase):', fullAdminError);
                        throw fullAdminError;
                    }

                    console.log('Admin başarıyla oluşturuldu (Supabase):', admin.id);
                    return this.mapAdminFromDb(fullAdmin);
                } catch (supabaseError: any) {
                    console.error('Supabase ile admin oluştururken hata:', supabaseError);
                    throw supabaseError;
                }
            }
        } catch (error: any) {
            console.error('Admin oluşturma işlemi sırasında hata:', error);
            throw error; // Tüm hataları yukarı ilet
        }
    }
    ...
    /**
     * Gets an admin by ID (alias for findAdminById for better naming consistency)
     * 
     * @param id Admin ID
     * @returns Admin data or null if not found
     */
    ...
    async findAdminByUsername(username: string): Promise<Admin | null> {
        try {
            ...
            try {
                ...
                // Prisma hata verirse devam ederiz, ama hata atmayız
            }

            // Prisma başarısız olursa veya kullanıcı bulunamazsa Supabase'e bakılır
            try {
                const { data: admin, error } = await supabase
                    .from('Admin')
                    .select(`
                ...
                console.error('Error finding admin by username with Supabase:', supabaseError);
                // Supabase hata verirse devam ederiz, ama hata atmayız
            }

            // Kullanıcı hiçbir veritabanında bulunamadı
            return null;
        } catch (error: any) {
            console.error('Error finding admin by username:', error);
            throw new Error(`Kullanıcı adını kontrol ederken bir hata oluştu: ${error.message}`);
        }
    }
    ...
    async findAdminByEmail(email: string): Promise<Admin | null> {
        try {
            ...
            try {
                ...
                console.error('Error finding admin by email with Supabase:', supabaseError);
                // Supabase hata verirse devam ederiz, ama hata atmayız
            }

            // E-posta ile bir kullanıcı hiçbir veritabanında bulunamadı
            return null;
        } catch (error: any) {
            console.error('Error finding admin by email:', error);
            throw new Error(`E-posta adresini kontrol ederken bir hata oluştu: ${error.message}`);
        }
    }
    ...
    async createInitialSuperAdmin(username: string, password: string, email?: string): Promise<Admin | null> {
        try {
            ...
            const hasSuperAdmin = await this.hasSuperAdmin();
            if (hasSuperAdmin) {
                console.log('Sistemde zaten bir superadmin bulunmaktadır, yeni superadmin oluşturulamaz');
                return null;
            }

            // Direkt createAdmin metodunu süper admin rolü belirterek çağır
            // NOT: Bu senaryo için özel olarak creatorRole parametresi gönderilmez
            ...
            console.error('Superadmin oluştururken hata:', error);
            return null;
        }
    }
