...
const categories = [
  ...
  { url: 'https://www.sporx.com/basketbol/', sportId: '5dc3ebe8-3111-47e3-86d4-648cc1c1df98' }, // Basketbol ID (Spor tablosundaki gerçek ID)
  { url: 'https://www.sporx.com/voleybol/', sportId: 'c82d3ffe-340e-494e-92ee-4e43ab376d8c' }, // Voleybol ID (Spor tablosundaki gerçek ID)
      ...
      try {
        ...
        for (let i = 0; i < Math.min(newsLinks.length, 10); i++) {
          ...
          try {
            ...
            // İçeriği al ve resimleri hiyerarşik olarak ekle
            let content = '';
            const contentElements = newsPage('.news-detail .news-content, .article-body, .story-content, .content-text, .article-text');
            
            // İçerik paragraflarını ve resimlerini hiyerarşik sırayla ekleyelim
            contentElements.find('p, img, figure').each((_, element) => {
              const el = newsPage(element);
              
              // Eğer bu bir resimse
              if (el.is('img') || el.find('img').length > 0) {
                const imgEl = el.is('img') ? el : el.find('img');
                const imgSrc = imgEl.attr('src');
                if (imgSrc) {
                  // Resmi [] içinde ekle
                  content += `[${formatUrl(imgSrc, category.url)}] `;
                }
              } 
              // Eğer bu bir paragrafsa
              else if (el.is('p')) {
            ...
            // Eğer içerik boşsa, eski yöntemi kullan
            if (!content.trim()) {
              content = newsPage('.news-detail .news-content, .article-body, .story-content, .content-text, .article-text').text().trim();
            }
            
            // Ana resmi her durumda içeriğin başına ekle
            const textContent = content;
            if (imageUrl) {
              content = `[${formatUrl(imageUrl, category.url)}] ${textContent}`;
            }
            ...
            if (title && content) {
              // Content çok kısaysa kaydetme
              if (!content || content.trim().length < 20) {
                console.log(`Haber içeriği çok kısa (${content ? content.length : 0} karakter) veya boş, kaydedilmiyor: ${title}`);
                continue;
              }
...
async function checkIfNewsExists(sourceUrl: string): Promise<boolean> {
    try {
        // Kaynak URL'e göre haber kontrolü yap
        const result = await newsService.findNewsBySourceUrl(sourceUrl);
        
        // Eğer haber bulunduysa true döndür
