  ...
  try {
    ...
    for (const site of konyasporSites) {
      ...
      try {
        ...
        console.log(`HTML içeriği boyutu: ${response.data.length} karakter`);
...
/**
 * Türkçe tarih metnini tarih nesnesine çevirir
 */
function parseDate(dateText: string): Date | null {
  // Temizle ve normalleştir
  dateText = dateText.toLowerCase().trim();
  
  // Bugün, dün gibi göreceli tarihleri işle
  if (dateText.includes('bugün')) {
    return new Date();
  }
  
  if (dateText.includes('dün')) {
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    return yesterday;
  }
  
  // Türkçe ay adlarını içeren düzenli ifade
  const dateRegex = /(\d{1,2})\s+(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık)\s+(\d{4})/i;
  const match = dateText.match(dateRegex);
  
  if (match) {
    const day = parseInt(match[1]);
    const month = parseMonthNameTR(match[2]);
  ...
  // Sayısal tarih formatı (ör: 10.04.2023)
  const numericDateRegex = /(\d{1,2})[./-](\d{1,2})[./-](\d{4})/;
  const numericMatch = dateText.match(numericDateRegex);
  
  if (numericMatch) {
    const day = parseInt(numericMatch[1]);
    const month = parseInt(numericMatch[2]) - 1; // JS'de aylar 0-11 arası
  ...
  // Tarih çıkarılamadıysa şu anki zamanı döndür
  return null;
}
